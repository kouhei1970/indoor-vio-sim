...
階をまたぐ経路では、視覚 SLAM のドリフトとループ閉じ込みが問題になります。
建物プリセットは階段室・吹抜を通って上下階を行き来できるので、
「同じ階に戻ってきたときにどれだけ位置がずれているか」を真値と比較できます。

```bash
# 学校校舎 (3 階建) の各階廊下を巡回する
node tools/render_dataset.mjs --out ./exp-building \
  --building school --pattern route --route patrol \
  --frames 600 --fps 20 --speed 0.8 --seed 1 --name school-patrol

# 同じ経路を逆向き・別シード (家具配置が変わる) で
node tools/render_dataset.mjs --out ./exp-building \
  --building school --pattern route --route patrol \
  --frames 600 --fps 20 --speed 0.8 --seed 2 --name school-patrol-s2
```
...
| `--building` | `--route` | 内容 |
|---|---|---|
| `school` | `patrol` / `classroom` | 3 階建校舎。階段室の吹抜で上下階へ / 1F 教室を順に回る |
| `community` | `patrol` | 2 階建公民館。2 層吹抜のロビーを通って 2F へ |
| `factory` | `inspection` / `patrol` | 工場。設備点検 / 生産エリアとキャットウォークの巡回 |
| `office` | `patrol` | 3 階建事務所ビル。西側コアの吹抜で上下階へ |

階ごとに床の材質と照明が違うので、「見た目が似ている階を取り違えるか」
(perceptual aliasing) の評価にも使えます。事務所ビルは全階が同じ間取り・
同じ内装なので、階の取り違えが起きやすい条件になります。
...
「点検対象がカメラに何フレーム写ったか」「そのときの距離と視野角」を
真値ポーズから計算できます。
...
# 点検対象の位置は建物プリセット側にあるので、必要なら手元でも持っておく
targets = {'P-101': [-9, 1.6, 2], 'J-12': [-5, 5.7, -7]}

t, p, q = load_tum('groundtruth_camera.txt')     # 1 節の関数
for name, xyz in targets.items():
    d = np.linalg.norm(p - np.array(xyz), axis=1)
    print(f'{name}: 最接近 {d.min():.2f} m  (フレーム {d.argmin()})')
```

点検飛行では狭い場所を低速で飛ぶため、地面効果・壁効果が効きます。
これらは物理側で再現しているので、点検姿勢を保つのに必要な制御余裕の
検討にもそのまま使えます。

### 2.7 NeRF / 3D Gaussian Splatting 用のデータ
...
- 建物の壁・階段・間取りはシードに依存しません (家具と掲示物だけが変わります)。
  同じ建物・同じ経路で、見た目の情報量だけを変えた比較ができます。
...
- プロペラ: 半径 14.99 mm・3 枚羽根 (公式 3D モデルより)
- 見た目は実機 CAD (STL) をそのまま使用 (`assets/stampfly/`)

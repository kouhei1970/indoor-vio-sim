...
機体プリセット `stampfly` は M5Stack StampFly の公表仕様に合わせてあります
(36.8 g / 82 mm 角 / 716-17600KV コアレスモータ / 1S 300 mAh HV)。
...
- **カメラ** — 実機には無いので、既定では 0.9 g の小型カメラを機首上部に
  載せた構成にしています。実際に載せるモジュールの質量・取り付け位置・
  俯角を「機体 → パーツ → カメラ (機体側)」で合わせてください。
  カメラを無効にすると質量はちょうど 36.8 g になります。
- **プロペラガード** — 別売りなので既定では付いていません。
  「パーツ → プロペラガード → 有効」で付けられます (+5.7 g)。
- **プロペラ直径** — 公表値が見つからなかったため、外形 82 mm 角に
  4 発を干渉なく収める幾何条件から 40 mm としています。
  実測値が分かれば `parts.prop.diameter` と `frame.armLength` を直してください。
- **推力係数** — `parts.prop.ct` / `cq` は小型プロペラの一般値です。
  推力試験のデータがあれば、ホバリング回転数が実測と合うよう `ct` を調整すると、
  スロットルと推力の関係が実機と揃います
  (現状: ホバリング約 33,000 rpm / 全開 56,800 rpm / 推力重量比 3.05)。

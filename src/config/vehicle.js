...
export const PRESETS = {
  ...
  stampfly: {
    ...
    power: {
      ...
      // 軸出力 → 電池電力の変換効率。実機のホバリング飛行時間 (320mAh で
      // 約 4 分) から逆算した。内訳の目安:
      //   ホバリング軸出力          7.4 W (34,000rpm・Cq=4.10e-11 の実測から)
      //   実測モータ定数の銅損       2.3 W (Rm=0.63Ω → モータ単体効率 77%)
      //   ブラシ摩擦・風損・駆動段    6.8 W (コアレスモータの無負荷電流ぶん。
      //                                    8520 級は 1 基 0.3〜0.5A ある)
      //   → 電池が出す電力 約 16.5 W = 4.3 A。320mAh をほぼ使い切って約 4 分
      // 銅損だけを見て 32% としていたが、それでは 2.6 分にしかならなかった。
      systemEfficiency: 0.45,
      avionicsPower: 0.6,
